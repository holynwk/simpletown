...
import com.simpletown.service.BlueMapService;
...
public class WarManager {
    ...
    public void setBlueMapService(BlueMapService blueMapService) {
    ...
    private void applyOccupiedTransfers(WarConflict conflict) {
        ...
        transferChunks(defender, attacker, conflict.getAttackerOccupied());
        transferChunks(attacker, defender, conflict.getDefenderOccupied());
        refreshBlueMap();
    }

    private void refreshBlueMap() {
        if (blueMapService != null) {
            blueMapService.refreshAll();
        }
    }

    private void transferChunks(Town from, Town to, Set<ChunkPosition> positions) {
        ...
        for (WarConflict conflict : getDistinctConflicts()) {
            ...
            for (WarFlag flag : new ArrayList<>(conflict.getActiveFlags())) {
                ...
                if (flag.getRemainingCaptureMs() <= 0) {
                    ...
                    clearHologram(flag);
                    conflict.getActiveFlags().remove(flag);
                }
            }
        }
        save();
    ...
    public boolean finishWithResult(Player player) {
        ...
        if (loser != null && winner != null) {
            transferChunks(loser, winner, loser.getChunks());
            double amount = loser.getBank() * 0.25;
            townManager.withdraw(loser, amount);
            townManager.deposit(winner, amount);
            townManager.deleteTown(loser);
        }
