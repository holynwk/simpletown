...
public class JobsService {
    ...
    public JobsService(SimpleTownPlugin plugin, JobsManager jobsManager, MessageService messages, TownManager townManager) {
        this.plugin = plugin;
    ...
    public boolean canTakeKillerJob(Player player) {
        if (player == null) {
            return false;
        }
        ...
        return !getCandidateTargets(player.getUniqueId(), null).isEmpty();
    }

    private Player pickTarget(UUID killerId, UUID lastTarget) {
        List<Player> candidates = getCandidateTargets(killerId, lastTarget);
    ...
    private List<Player> getCandidateTargets(UUID killerId, UUID lastTarget) {
        List<Player> candidates = new ArrayList<>();
        Player killer = Bukkit.getPlayer(killerId);
        Town killerTown = killer == null ? null : townManager.getTownByMember(killer.getName());
        for (Player online : Bukkit.getOnlinePlayers()) {
            ...
            Town onlineTown = townManager.getTownByMember(online.getName());
            if (killerTown != null && onlineTown != null && killerTown.getName().equalsIgnoreCase(onlineTown.getName())) {
                continue;
            }
