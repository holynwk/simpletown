...
public class BlueMapService {
    ...
    private final java.util.Map<String, Set<String>> occupiedMarkers = new ConcurrentHashMap<>();
    private BlueMapAPI api;

    public BlueMapService(JavaPlugin plugin, TownManager townManager, WarManager warManager) {
        this.plugin = plugin;
    ...
    private void drawTown(Town town) {
        ...
        BlueMapAPI.getInstance().ifPresent(apiLocal -> {
            for (BlueMapMap map : apiLocal.getMaps()) {
                ...
                java.util.function.BiConsumer<java.util.Map.Entry<Integer, Integer>, java.util.Map.Entry<Integer, Integer>> addEdge = (a, b) -> {
                ...
                while (!remaining.isEmpty()) {
                    ...
                    while (true) {
                        final int bx = curr.bx();
                        final int bz = curr.bz();

                        Edge next = remaining.stream()
                                .filter(e -> e.ax() == bx && e.az() == bz)
                                .findFirst()
                                .orElse(null);
    ...
    private void drawOccupiedChunks() {
        if (warManager == null) {
            return;
        }
        clearOccupied();
        BlueMapAPI.getInstance().ifPresent(apiLocal -> {
            Set<String> newIds = new HashSet<>();
            for (WarConflict conflict : new java.util.HashSet<>(warManager.getConflicts())) {
                if (conflict.getStatus() == com.simpletown.war.WarStatus.ENDED) {
                    continue;
                }
                addOccupiedMarkers(conflict.getAttacker(), conflict.getAttackerOccupied(), apiLocal, newIds);
                addOccupiedMarkers(conflict.getDefender(), conflict.getDefenderOccupied(), apiLocal, newIds);
            }
            occupiedMarkers.put("occupied", newIds);
        });
    }


    private void addOccupiedMarkers(String controller, Set<ChunkPosition> chunks, BlueMapAPI apiLocal, Set<String> newIds) {
        if (chunks.isEmpty()) {
            return;
        }
        Town controllerTown = townManager.getTownByName(controller);
        Color base = colorFromHex(controllerTown == null ? null : controllerTown.getMapColor());
        Color fillCol = new Color(base.getRed(), base.getGreen(), base.getBlue(), 0.4f);
        Color outlineCol = new Color(base.getRed(), base.getGreen(), base.getBlue(), 1f);
        for (BlueMapMap map : apiLocal.getMaps()) {
            MarkerSet set = map.getMarkerSets().get(MARKER_SET_ID);
            ...
            for (ChunkPosition pos : chunks) {
                ...
                String fillId = "occupied-" + controller + "-" + pos.getX() + "-" + pos.getZ();
                set.getMarkers().put(fillId, ShapeMarker.builder()
                        .label("Оккупировано " + controller)
                        .shape(rect, HEIGHT_Y)
                        .fillColor(fillCol)
                        .lineColor(outlineCol)
                        .depthTestEnabled(false)
    ...
    private void clearOccupied() {
        BlueMapAPI.getInstance().ifPresent(apiLocal -> {
            Set<String> ids = occupiedMarkers.remove("occupied");
            if (ids == null || ids.isEmpty()) {
                return;
            }
            for (BlueMapMap map : apiLocal.getMaps()) {
