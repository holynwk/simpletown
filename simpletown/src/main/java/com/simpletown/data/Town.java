...
public class Town {
    ...
    private final Map<ChunkPosition, PlotData> plots;
    ...
    public Town(String name, String mayor, Set<String> citizens, Set<ChunkPosition> chunks, ChunkPosition capital, boolean open, TownFlags citizenFlags, TownFlags outsiderFlags, String mapColor) {
        this(name, mayor, citizens, chunks, capital, 0.0, open, citizenFlags, outsiderFlags, mapColor, AgeTier.AGE1.getLevel(), new HashMap<>(), new ArrayList<>(), "", new HashMap<>(), null, System.currentTimeMillis());
    }
    ...
    public Town(String name, String mayor, Set<String> citizens, Set<ChunkPosition> chunks, ChunkPosition capital, double bank, boolean open, TownFlags citizenFlags, TownFlags outsiderFlags, String mapColor) {
        this(name, mayor, citizens, chunks, capital, bank, open, citizenFlags, outsiderFlags, mapColor, AgeTier.AGE1.getLevel(), new HashMap<>(), new ArrayList<>(), "", new HashMap<>(), null, System.currentTimeMillis());
    }
    ...
    public Town(String name, String mayor, Set<String> citizens, Set<ChunkPosition> chunks, ChunkPosition capital, double bank, boolean open, TownFlags citizenFlags, TownFlags outsiderFlags, String mapColor, int ageLevel, Map<BuildingType, Integer> buildingLevels, List<org.bukkit.inventory.ItemStack> inventoryContents) {
        this(name, mayor, citizens, chunks, capital, bank, open, citizenFlags, outsiderFlags, mapColor, ageLevel, buildingLevels, inventoryContents, "", new HashMap<>(), null, System.currentTimeMillis());
    }

    public Town(String name, String mayor, Set<String> citizens, Set<ChunkPosition> chunks, ChunkPosition capital, double bank, boolean open, TownFlags citizenFlags, TownFlags outsiderFlags, String mapColor, int ageLevel, Map<BuildingType, Integer> buildingLevels, List<org.bukkit.inventory.ItemStack> inventoryContents, String boardMessage, Map<ChunkPosition, PlotData> plots, Location spawn, long createdAt) {
        this.name = name;
        ...
        this.boardMessage = boardMessage == null ? "" : boardMessage.trim();
    ...
    public void setMayor(String mayor) {
        if (mayor != null && !mayor.trim().isEmpty()) {
            this.mayor = normalizeName(mayor);
        }
    }
    ...
    public Map<ChunkPosition, PlotData> getPlots() {
        return Collections.unmodifiableMap(plots);
    }

    public PlotData getPlot(ChunkPosition position) {
        return plots.get(position);
    }

    public void setPlot(ChunkPosition position, PlotData plot) {
        if (position == null || plot == null) {
            return;
        }
        plots.put(position, plot);
    }

    public void removePlot(ChunkPosition position) {
